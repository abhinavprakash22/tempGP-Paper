function[] = tsKnn_ext(turbine)
%setting up the output file name
outfile = strcat('./intermediate_results/Turbine',num2str(turbine),'_tsKnn_results.txt');
logfile = strcat('./intermediate_results/Turbine',num2str(turbine),'_tsKnn.log');
fprintf('%s %s\n','Executing TS-kNN code for Turbine',num2str(turbine));
...
options = struct('foldType',"tempFold",'subsetSelection',true);
knnObj = fitknn(trainX,trainY,options); %fitting time-split kNN
