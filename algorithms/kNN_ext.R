################################################################################
#DESCRIPTION: extended case study file for kNN for temporal overfitting paper
################################################################################
...
outfile = paste0("./intermediate_results/Turbine",turbine,"_knn_results.txt")
logfile = paste0("./intermediate_results/Turbine",turbine,"_knn.log")
...
data$time_stamp=as.character(data$time)
...
bestSubsetResult = computeBestSubset(CVdataset = CVdataset, ycol = ycol, covariates = covariates, verbose = verbose)
...
for (i in 1:2){
  ...
  if (verbose){
    cat('RMSE for kNN model for Turbine',turbine,'for test dataset',test_names[i],'is: ',rmse,'\n', file = logfile, append = TRUE)
  }
